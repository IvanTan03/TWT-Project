...
document.addEventListener('DOMContentLoaded', (event) => {
    function checkAnswer(questionNumber, correctAnswerIndex, event) {
        const modal = document.getElementById('modal');
        const modalText = document.getElementById('modal-text');
        const retryButton = document.getElementById('retry');
        const nextButton = document.getElementById('next');

        const selectedAnswerIndex = Array.from(event.target.parentNode.children).indexOf(event.target);
        ...
        if (correctAnswers[questionNumber - 1].includes(selectedAnswerIndex)) {
    ...
    window.goBackToExercise5 = goBackToExercise5;
});
